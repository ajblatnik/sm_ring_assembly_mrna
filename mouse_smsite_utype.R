...
## processed to generate a single master file yielding the frequency of Sm-sites identified in the Canonical transcript of a gene,
...
# create a table to cross-reference refseq and ensembl ids
  # access BioMart and find attributes of the database
    ...
    ensembl_m <- useDataset("mmusculus_gene_ensembl", mart = ensembl)
    attributes <- listAttributes(ensembl_m)
  # create table with ensembl_gene_id, ensembl_transcript_id, transcript_is_canonical, refseq_mrna, refseq_ncrna, transcript_mane_select, gene_biotype
    refseq <- getBM(attributes = c("ensembl_gene_id", "ensembl_transcript_id", "transcript_is_canonical", "refseq_mrna", "refseq_ncrna", "gene_biotype"),
                    mart = ensembl_m,
                    useCache = F) 
    write_tsv(as.data.frame(refseq), file = "/Users/dapperwhiskyman/Desktop/deseq2/from_BioMart/m_refseq.tsv", quote = "none")
  # create table with ensembl_transcript_id, transcript_length, 5_utr_start, 5_utr_end, CDS_length, 3_utr_start, 3_utr_end)
    ...
    # make 'NA' = '0'
      lengths <- lengths %>% mutate(across(everything(), ~replace_na(., 0)))
      ...
      write_tsv(as.data.frame(lengths_final), file = "/filepath/m_lengths.tsv", quote = "none")
  ...
  # inner_join refseq with lengths_final by ensembl_transcript_id since the same number of transcript_ids
    ...
    # there are no MANE transcripts for mouse, therefore make a table of just transcript_is_canonical = 1
      tisc <- refleng %>% filter(transcript_is_canonical == "1")
        ...
        # convert empty strings to NA
          ...
          # replace NA values in refseq_mrna with corresponding values from refseq_ncrna, keeping refseq_mrna values if they exist
            ...
            # remove uneccessary columns and rename refseq_mrna to refseq_id
              tisc <- tisc[c(1:2,4,6:10)]
              names(tisc)[3] <- "refseq_id"
              write_tsv(as.data.frame(tisc), file = "/filepath/m_refseq_master.tsv", quote = "none")
...
# import mouse refseq and gencode sm-site output files - called u1g for U1 Sm-site from gencode

  input_dir_r <- "/filepath to directory containing refseq output from find_smsites.py"
  input_dir_g <- "/filepath to directory containing gencode output from find_smsites.py"
  output_dir_r <- "/filepath to desired output directory for refseq files"
  output_dir_g <- "/filepath for desired output directory for gencode files"

  # get all .txt files in the input directory
  txt_files_r <- list.files(input_dir_r, pattern = "*_stem_loops_nih_mouse.txt", full.names = TRUE)
  txt_files_g <- list.files(input_dir_g, pattern = "*_stem_loops_gencode_mouse.txt", full.names = TRUE)
  
  remove_suffix <- function(x) {sub("\\.[^.]*$", "", x)}
  
  # Loop through each refseq file
  ...
  # Loop through each gencode file
...
# now to combine tables and make one master smsite list that crossrefrences both annotations
  input_dir_r2 <- "/filepath to refseq output of preceding code"
  input_dir_g2 <- "/filepath to gencode output of preceding code"
  ...
  # get all .txt files in the input directory
    txt_files_r2 <- list.files(input_dir_r2, pattern = "*_nih_mouse.tsv", full.names = TRUE)
    txt_files_g2 <- list.files(input_dir_g2, pattern = "*_gencode_mouse.tsv", full.names = TRUE)
  ...
  # for each, keep only those refseq_ids or transcript_ids present in the refseq_master table, then full_join by sequence
    ur <- ur %>% filter(refseq_id %in% tisc$refseq_id) 
    ug <- ug %>% filter(ensembl_transcript_id %in% tisc$ensembl_transcript_id) 
  # full_join by sequence column
    ...
    # some rows are duplicate sequences but the annotated positions between refseq and gencode are different, and gene_name.x is different from gene_name.y
      # make a table of only unique sequences
        ...
        # remove empty columns and then merge with tisc
          uf_unique_donot <- uf_unique_donot[,c(1:6)]  
          uf_unique_donot <- inner_join(uf_unique_donot, tisc, by = "refseq_id") 
      ...
      # bind_rows
        ...
        uf_final <- bind_rows(uf_final, uf_duplicate6) 
        ...
        # Generate output filename using the first two characters of the basename
          ...
          output_filename <- paste0(output_dir, "/", substr(base_name, 1, 2), "_mouse.tsv")  
...
# compile a single master table from refseq_final (h_refseq_master or m_refseq_master) that provides smsite-frequency data
  ...
  u1 <- read_delim("/filepath to u1_mouse.tsv", delim = "\t", escape_double = FALSE, trim_ws = TRUE)
  u2 <- read_delim("/fileptath to u2_mouse.tsv", delim = "\t", escape_double = FALSE, trim_ws = TRUE)
  u5 <- read_delim("/filepath to u5_mouse.tsv", delim = "\t", escape_double = FALSE, trim_ws = TRUE)
  u7 <- read_delim("/filepath to u7_mouse.tsv", delim = "\t", escape_double = FALSE, trim_ws = TRUE)
  pe <- read_delim("/filepath to noncanonical_mouse.tsv", delim = "\t", escape_double = FALSE, trim_ws = TRUE)
    ...
    # rename columns for each
      ...
      # full_join to make a comprehensive table
        ...
        # replace all NA with 0
          ...
          replace_na_pe <- is.na(smsite$pe_freq)
            smsite$pe_freq[replace_na_pe] <- 0
          # make a new column with the difference between perm_freq and the rest, to make a column of truly permissive smsites
            ...
            # remove perm_freq column
              ...
              # covert all negative values to absolute values (pe does not pick up all sm-types)
                smsite <- smsite %>% mutate(across(where(is.numeric), abs))
  ...
  # append smsite frequency for each region of rna
    ...
    # combine with smsite 
      ...
      #combine with tisc
        master <- full_join(tisc, smsite_region, by = "ensembl_transcript_id") 
        # replace all NA with 0
          ...
          # reorder to double check frequencies
            ...
            # make a column combining U1, U2, and U5 freq to be canon_freq, and also canon_3utr_freq
              ...
              master <- master %>% mutate(canon_3utr = u1_3utr + u2_3utr + u5_3utr)
                write_tsv(as.data.frame(master), file = "/filepath/mouse_smsite_master.tsv", quote = "none")
